...
typedef struct svector {
  WORD_NEW    *words;              /* The features/values in the vector by
  ...
  char    *userdefined;        /* You can put additional information
				  ...
				  the input file. */
  int    kernel_id;           /* Feature vectors with different//64bit long->int
...
typedef struct doc {
  int    docnum;              /* Document ID. This has to be the position of //64bit long->int
                                  the document in the training set array. */
  int    queryid;             /* for learning rankings, constraints are //64bit long->int
  ...
  int    slackid;             /* Index of the slack variable//64bit long->int
...
typedef struct kernel_parm {
  int    kernel_type;   /* 0=linear, 1=poly, 2=rbf, 3=sigmoid, 4=custom *///64bit long->int
  int    poly_degree;//64bit long->int
  double  rbf_gamma;
  double  coef_lin;
  double  coef_const;
  char    custom[50];    /* for user supplied kernel */
    //20110617 TK AS for win.KERNEL_PARM.length = mac.KERNEL_PARM.leng + 4
	char    add_dif_win_mac[4];
	//20110617 TK AE
} KERNEL_PARM;

typedef struct model {
  float  *feature;
  int    sv_num;	//64bit long->int
  int    at_upper_bound;//64bit long->int
  double  b;
  DOC     **supvec;
  double  *alpha;
  int    *index;       /* index from docnum to position in model *///64bit long->int
  int    totwords;     /* number of features *///64bit long->int
  int    totdoc;       /* number of training documents *///64bit long->int
